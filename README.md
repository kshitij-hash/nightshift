...
NIGHTSHIFT adds recurrence without asking the pool for anything it does not
have.
...
subscriber already parted with, plus rules on when and how often it can move.
There is nothing to steal, revoke, or phish: a keeper who fires a charge can
only make the vault do exactly what the subscriber committed to. Designs that
guard a live key with policy rules can fail two ways: the policy gets
misconfigured, or the key leaks. Here no key with spending power exists at
all. The escrow moves on a hash the subscriber precomputed, or it does not
move.
...
## Anyone can submit a cancel

A cancel is authorized by a STARK signature from the subscription's owner key
over `cancel_message(commitment)`. The vault checks the signature and never
reads the sender, so the subscriber signs and any willing party submits. A
cancel costs the subscriber no gas and reveals no sender identity. For
revocation this covers the RFP's paymaster role with less trust than a
paymaster: the relay holds nothing and can change nothing. If it alters the
...
- **Fixed price and period steps.** Amounts and period lengths come from a
  small fixed ladder, so a schedule cannot fingerprint its subscriber.
- **Accounted custody.** `received = balance_of(self) − accounted[token]`,
  asserted equal to the schedule's exact price, so a stray donation cannot
  mint a subscription (tested).
...
Session keys are not implemented, deliberately: the pool has nothing to build
them on, so the standing authorization is escrow the subscriber already
