 ...
 *  then two public keyless nodes as failover.
 *
 ...
 *  a browser needs. The list used to hold BlastAPI and then Lava; both now
 *  answer every request with "discontinued", so they are gone rather than
 *  costing every cold load two failed round trips before a live node answers. */
